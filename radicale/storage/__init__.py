...
CACHE_VERSION: bytes = "".join("%s=%s;" % (pkg, pkg_resources.get_distribution(
    pkg).version) for pkg in CACHE_DEPS).encode()
...
class BaseCollection:
    ...
    def get_multi(self, hrefs: Iterable[str]) -> Iterable[Tuple[
            str, Optional["radicale_item.Item"]]]:
    ...
    def get_filtered(self, filters: Iterable[ET.Element]) -> Iterable[
            Tuple["radicale_item.Item", bool]]:
    ...
    @overload
    def get_meta(self, key: None = None) -> Mapping[str, str]:  # noqa: F811
        ...

    @overload
    def get_meta(self, key: str) -> Optional[str]:  # noqa: F811
        ...

    def get_meta(self, key=None):  # noqa: F811

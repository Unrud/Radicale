...
class ParallelHTTPServer(socketserver.ThreadingMixIn,
    ...
    server_address: ADDRESS_TYPE  # type: ignore[assignment]

    def __init__(
            self, configuration: config.Configuration, family: int,
            address: Tuple[str, int], RequestHandlerClass: Callable[
                ..., http.server.BaseHTTPRequestHandler]) -> None:
        self.configuration = configuration
        self.address_family = family
        super().__init__(address, RequestHandlerClass)
        self.worker_sockets: Set[socket.socket] = set()
    ...
    def get_request(  # type: ignore[override]
        ...
        timeout: float = self.configuration.get("server", "timeout")
        if timeout > 0:
    ...
    def verify_request(  # type: ignore[override]
            self, request: socket.socket, client_address_and_socket: Tuple[
                ADDRESS_TYPE, socket.socket]) -> bool:
        return True

    def process_request(  # type: ignore[override]
            self, request: socket.socket, client_address_and_socket: Tuple[
                ADDRESS_TYPE, socket.socket]) -> None:
    ...
    def handle_error(  # type: ignore[override]
            ...
            client_address_or_client_address_and_socket: Union[
                ADDRESS_TYPE, Tuple[ADDRESS_TYPE, socket.socket]]) -> None:

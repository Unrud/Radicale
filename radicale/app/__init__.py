...
from radicale import httputils, log, pathutils, types
...
class Application(ApplicationPartDelete, ApplicationPartGet,
                  ApplicationPartHead, ApplicationPartMkcalendar,
    ...
    def _headers_log(self, environ: types.WSGIEnviron) -> types.WSGIEnviron:
        """Sanitize headers for logging."""
        request_environ = dict(environ)

        # Mask passwords
        mask_passwords: bool = self.configuration.get("logging",
                                                      "mask_passwords")
    ...
    def _handle_request(self,
                        environ: types.WSGIEnviron) -> _IntermediateResponse:
        """Manage a request."""
        ...
        logger.debug("Request headers:\n%s",
                     pprint.pformat(self._headers_log(environ)))
        ...
        if user and login == user:
            ...
            delay: float = self.configuration.get("auth", "delay")
            if delay > 0:
        ...
        if self.configuration.get("server", "_internal_server"):
            ...
            if content_length:
                max_content_length: int = self.configuration.get(
                    "server", "max_content_length")
                if (max_content_length > 0 and
        ...
        if ((status, headers, answer) == httputils.NOT_ALLOWED and not user and
            ...
            realm: str = self.configuration.get("auth", "realm")
            headers = dict(headers)
            headers.update({
                "WWW-Authenticate":
                "Basic realm=\"%s\"" % realm})
